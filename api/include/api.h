...
// todo id counter from what is already on disk, and build the read cache.
...
//
//   GET  /api/hello    GET  /api/status    GET /api/cache
//   POST /api/echo     GET  /api/stats     GET /ws/chat
//   /api/todos[/{id}]  -- full CRUD, see todo_handlers.h
void api_mount(http_server_t *server);

// Tear the API down: drop the read cache and checkpoint/close the storage
// engine. Call once the server has stopped serving, so no handler can be
// running.
